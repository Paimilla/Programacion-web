<?PHP
    // recibir datos del formulario candidatos.html
    $rut= $_POST['rut'];
    $nombre = $_POST['nombre'];
    ...
    $validar = "SELECT * FROM votantes WHERE rut = '$rut'";
    $validando = mysqli_query($conn,$validar);
    if(mysqli_num_rows($validando) > 0){
        echo '<script>
        alert("Oh ah habido un problema, el rut ingresado ya esta registrado ");
        window.history.go(-1);
        </script>';
        exit;
    }
    $sql="INSERT INTO votantes(rut,nombre,apellido_p,apellido_m,direccion) VALUES ('$rut','$nombre','$apellido_p','$apellido_m','$direccion')";
