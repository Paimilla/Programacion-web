...
$validar = "SELECT * FROM votacion WHERE rut_votante = '$rut'";
$validando = mysqli_query($conn,$validar);
if(mysqli_num_rows($validando) > 0){
    echo '<script>
    alert("Oh ah habido un problema, el rut ingresado ya ha votado ");
    window.history.go(-1);
    </script>';
    exit;
}
$validar = "SELECT rut FROM votantes WHERE rut = '$rut'";
$validando = mysqli_query($conn,$validar);
if(mysqli_num_rows($validando) == 0){
    echo '<script>
    alert("Oh ah habido un problema, el rut ingresado no esta registrado ");
    window.history.go(-1);
    </script>';
    exit;
}

$sql="INSERT INTO votacion (rut_votante,codigocandidato,fecha_votacion) VALUES ('$rut','$voto','$fechaActual')";
?>
<!DOCTYPE html>
<html>
